  ...
  content = templatefile("${path.module}/manifests/inventory-template.yaml",
    {
      ...
      name       = "${var.area}-registry"
  })
  filename        = "./ansible/inventory-registry.yaml"
...
resource "local_file" "playbook" {
  ...
  filename        = "./ansible/playbook-registry.yaml"
  file_permission = "0644"
}

resource "null_resource" "play_ansible" {
  provisioner "local-exec" {
    command = "ansible-playbook -i ansible/inventory-registry.yaml ansible/playbook-registry.yaml"
  }
  depends_on = [
...
output "registry_server_ip_address" {
  description = "REGISTRY IP Address"
